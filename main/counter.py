...
def compute_people(video, user_id):
    with app.app_context():
        ...
        while True:
            ...
            if has_frame:
                ...
                minarea = 450 #Area is subject to change depending on distance of camera from people
                maxarea = 10000
                    ...
                    if curcent:  # if there is a current centroid
                        ...
                        if oldcent:  # checks if old centroid exists
                            ...
                            if oldcent[1] >= lineypos2 and curcent[1] <= lineypos2 and PersonIDs[currentpeopleindex[i]] not in has_crossed:
                                if area > 10000:
                                    exiting = exiting + 2
                                    customer = Customer(date=datetime.now(), status="exit", user_id=user_id)
                                    customer2 = Customer(date=datetime.now(), status="exit", user_id=user_id)
                            ...
                            # to count persons and that person hasn't been counted yet
                                ...
                                if area > 10000:
                                    entering = entering + 2
                                    customer = Customer(date=datetime.now(), status="entry", user_id=user_id)
                                    customer2 = Customer(date=datetime.now(), status="entry", user_id=user_id)
                                    db.session.add(customer)
                                    db.session.add(customer2)
                                else:
                                    entering = entering + 1
